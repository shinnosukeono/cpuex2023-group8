...
public:
	...
	Data ReadIntRegisters(int i)
	{
		...
		if (forwarding_bit_EX_Read_int & (1 << i)){
			if (print_mode == 1 && cycle >= print_cycle && cycle <= print_cycle_end){
	...
	void eval(std::vector<Instruction *> &intrs)
	{
		...
		if (!fast_mode){
			vector<int> SaturatingCounter(intrs.size(), 0);
			vector<bool> SaturatingCounter_FSM(intrs.size(), false);
			...
			while (pc < 4 * (int)intrs.size() && pc != -1 && cycle < maxcount)
			{
				...
				if (IF_next_pc < 4 * (int)intrs.size() && IF_flag) {
					...
					IF_pc = IF_next_pc;
					if (prediction_mode == 1)
						IF_next_pc = SaturatingCounter[IF_next_pc / 4];
					else if (prediction_mode == 2){
				...
				if (EX != nullptr){
					...
					if (pc == (ID_pc)){
						SaturatingCounter_FSM[ex_pc/4] = true;
					...
					}else
					{
						...
						IF_flag = false;
						if (prediction_mode == 1){
							if (SaturatingCounter_FSM[ex_pc/4]) SaturatingCounter_FSM[ex_pc/4] = false;
							else SaturatingCounter[ex_pc/4] = pc;
						}else if (prediction_mode == 2){
				...
				cycle++;
				if (print_mode == 1 && cycle-1 >= print_cycle && cycle-1 <= print_cycle_end){
				...
				if (load_flag && ((forwarding_flag_int & forwarding_bit_MEM_int) || (forwarding_flag_float & forwarding_bit_MEM_float))){
					if (print_mode == 1 && cycle-1 >= print_cycle && cycle-1 <= print_cycle_end) outputFile << endl << endl << "---------------------------------------------------------stalled---------------------------------------------------------" << endl << endl;
					cycle += 1;
					stall_count++;
				}

				if (Flush_flag){
					if (print_mode == 1 && cycle-1 >= print_cycle && cycle-1 <= print_cycle_end) outputFile << endl << endl << "---------------------------------------------------------Flushed---------------------------------------------------------" << endl << endl;
					Flush_flag = false;
					flush_count++;
				}
				// outputFile << "Memory:{";
				...
				if (print_mode == 1 && cycle-1 >= print_cycle && cycle-1 <= print_cycle_end){
					...
					for (int i = 0; i < 32; i++)
					{
						...
						if (i % 8 == 0)
							outputFile << endl
									<< "		  ";
						outputFile << Rnames[i] << ":[" << get<int>(VirtualIntRegisters_EX[i]) << "]";
					}
					outputFile << "}" << endl;
				}
