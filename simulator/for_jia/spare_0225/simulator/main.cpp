...
const int sec_per_data = CLK_PER_HALF_BIT + CLK_PER_BIT*9;
...
int main()
{
	...
	double sec_of_data = (double)4*(data_length) * sec_per_data / (double)freq;
	sec_of_data += (double)cycle_of_data_section / (double)freq;
	cout << "sec_of_data : " << sec_of_data << " sec" << endl;
	cout << "cycle_of_data_section : " << cycle_of_data_section << " cycle" << endl;

	if (vm.fast_mode == 0){
		cout << "cycle (Dependency Optimization) : " << vm.cycle << endl;
		cout << "(Estimated executon time: " << (double)vm.cycle/(clock_rate*1e6) << " sec)" << endl;
		...
		cout << "(Estimated sum of time: " << (double)vm.cycle_no_dependency/(clock_rate*1e6) + sec_of_data+0.55 << " sec)" << endl;
