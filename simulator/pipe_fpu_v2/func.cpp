...
public:
	...
	Data ReadFloatRezisters(int i)
	{
		if (forwarding_bit_EX_Read_float & (1 << i)){
			outputFile << "\tForwarding: " << Fnames[i] << ":[" << get<float>(FloatRezisters[i]) << "]" << endl;
		...
		while (pc < 4 * (int)intrs.size() && pc != -1 && count < maxcount)
		{
			...
			if (EX != nullptr){
				if ((*EX).name.substr(0,2) == "Sw" || (*EX).name.substr(0,2) == "Lw" || (*EX).name.substr(0,2) == "Lh" || (*EX).name.substr(0,2) == "Lb" || (*EX).name.substr(0,2) == "Sh" || (*EX).name.substr(0,2) == "Sb" || (*EX).name.substr(0,3) == "Flw" || (*EX).name.substr(0,3) == "Fsw"){
					...
					if ((*EX).name.substr(0,3) == "Fsq") cout << "here!" << endl;
					(*EX).exec(*this);
				}
			...
			if (print_mode == 1 && load_flag && ((forwarding_flag_int & forwarding_bit_MEM_int) || (forwarding_flag_float & forwarding_bit_MEM_int))){
