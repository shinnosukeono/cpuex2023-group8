		...
		if (!fast_mode){
			while (pc < 4 * (int)intrs.size() && pc != -1 && cycle < maxcount)
			{
				...
				if (load_flag && ((forwarding_flag_int & forwarding_bit_MEM_int) || (forwarding_flag_float & forwarding_bit_MEM_float))){
					if (print_mode == 1 && cycle >= print_cycle && cycle <= print_cycle_end) outputFile << endl << endl << "---------------------------------------------------------stalled---------------------------------------------------------" << endl << endl;
					cycle += 1;
					stall_count++;
				}

				if (Flush_flag){
					if (print_mode == 1 && cycle >= print_cycle && cycle <= print_cycle_end) outputFile << endl << endl << "---------------------------------------------------------Flushed---------------------------------------------------------" << endl << endl;
					Flush_flag = false;
					flush_count++;
				}
				// outputFile << "Memory:{";
