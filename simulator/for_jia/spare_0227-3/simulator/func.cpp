...
extern long long clock_per_1byte_data;
...
public:
	...
	int fast_mode = 0;
	long long stall_count_dependency = 0;
	long long stall_count_no_dependency = 0;
...
public:
	int stalling_turn_int[32]={}, stalling_turn_float[32]={};
	vector<int> read_register_int, read_register_float;
	int write_number_int, write_number_float;
	...
	long long cycle_no_dependency = 0;
	bool print_flag = false;
	long long cin_last_cycle = 0;
	long long cin_count = 0;
	long long cin_stall_count = 0;
	
	Data ReadIntRegisters(int i)
	{
		if (i == 0)
			return Data{0};
		if (!fast_mode) read_register_int.push_back(i);
	...
	{
		if (!fast_mode) read_register_float.push_back(i);
			...
			while (pc < 4 * (int)intrs.size() && pc != -1 && cycle < maxcount)
			{
				if ((pc % 4) != 0){
					...
					if (dependency_penalty){
						cerr << "cycle: " << cycle_no_dependency << endl;
					}else{
				...
				last_pc = pc;
				if (dependency_penalty){
					last_cycle = cycle_no_dependency;
				}else{
					last_cycle = cycle;
				}
				if (!dependency_penalty){
					...
					if (print_mode > 0 && cycle_no_dependency >= print_cycle && cycle_no_dependency <= print_cycle_end){
						print_flag = true;
					}else{
						print_flag = false;
					}
				...
				if (print_flag){
					if (dependency_penalty)
						outputFile << endl << "cycle:[" << cycle_no_dependency << "], ";
					else
				...
				if (MEM != nullptr){
					if ((*MEM).name.substr(0,2) == "Sw" || (*MEM).name.substr(0,2) == "Sh" || (*MEM).name.substr(0,2) == "Sb" || (*MEM).name.substr(0,3) == "Fsw"){
						...
						if (!mem.hit_flag){
							if (mem.write_back_flag){
								cycle += cache_miss_time_wtb;
								cycle_no_dependency += cache_miss_time_wtb;
							}else{
								cycle += cache_miss_time_no_wtb;
								cycle_no_dependency += cache_miss_time_no_wtb;
							}

							if (print_flag){
								if (dependency_penalty)
									outputFile << "\tCache miss occurs, stall until cycle: " << cycle_no_dependency << endl;
								else
									outputFile << "\tCache miss occurs, stall until cycle: " << cycle << endl;
							}
						}
					}
					else if ((*MEM).name.substr(0,2) == "Lw" || (*MEM).name.substr(0,2) == "Lh" || (*MEM).name.substr(0,2) == "Lb" || (*MEM).name.substr(0,3) == "Flw"){
						mem.hit_flag = false;
						mem.write_back_flag = false;
						if (print_flag){
							outputFile << "\tMemory acsess: [" << (*MEM).name << "]" << endl;
				...
				// for finding stall
				read_register_int.clear();
				read_register_float.clear();
				write_number_int = -1;
				write_number_float = -1;
				...
				if (EX != nullptr){
					...
					int max_stall = cycle;
					for (auto i : read_register_int){
						max_stall= max(max_stall, stalling_turn_int[i]);
					}

					for (auto i : read_register_float){
						max_stall= max(max_stall, stalling_turn_float[i]);
					}

					if (load_flag && ((forwarding_flag_int & forwarding_bit_MEM_int) || (forwarding_flag_float & forwarding_bit_MEM_float))){
						cycle++;
						stall_count_dependency++;
					}

					if (max_stall > cycle){
						stall_count_dependency++;
						cycle = max_stall;
						if (!dependency_penalty && print_flag){
							outputFile << "\tStall occurs until cycle: " << cycle << endl;
						}
					}
					...
					if ((*EX).name.substr(0,2) == "Sw" || (*EX).name.substr(0,2) == "Lw" || (*EX).name.substr(0,2) == "Lh" || (*EX).name.substr(0,2) == "Lb" || (*EX).name.substr(0,2) == "Sh" || (*EX).name.substr(0,2) == "Sb" || (*EX).name.substr(0,3) == "Flw" || (*EX).name.substr(0,3) == "Fsw"){
						...
						cycle_no_dependency += 1;
					}else if ((*EX).name.substr(0,4) == "Fdiv" || (*EX).name.substr(0,5) == "Fsqrt" || (*EX).name.substr(0,5) == "Fmadd" || (*EX).name.substr(0,5) == "Fmsub" || (*EX).name.substr(0,6) == "Fnmadd" || (*EX).name.substr(0,6) == "Fnmsub"){
						if (write_number_float != -1){
							stalling_turn_float[write_number_float] = cycle+5;
						}
						cycle_no_dependency += 5;
					}else if ((*EX).name.substr(0,1) == "F"){
						cycle_no_dependency += 2;
						if (write_number_float != -1){
							stalling_turn_float[write_number_float] = cycle+2;
						}
						if (write_number_int != -1){
							stalling_turn_int[write_number_int] = cycle+2;
						}
					}
					else if ((*EX).name.substr(0,3) == "Cin"){
						cycle_no_dependency += 1;
						cin_count++;
						if (dependency_penalty){
							if (cycle_no_dependency < cin_count*4*clock_per_1byte_data){
								cin_stall_count += cin_count*4*clock_per_1byte_data+1 - cycle_no_dependency;
								cycle_no_dependency = cin_count*4*clock_per_1byte_data+1;
							}
						}else{
							if (cycle < cin_count*4*clock_per_1byte_data){
								cin_stall_count += cin_count*4*clock_per_1byte_data+1 - cycle;
								cycle = cin_count*4*clock_per_1byte_data+1;
							}
						}
						if (write_number_int != -1){
							stalling_turn_int[write_number_int] = cycle+1;
						}
					}else{
						stall_flag = false;
					}

					if (stall_flag && dependency_penalty){
						stall_count_no_dependency++;
						if (print_flag){
							outputFile << "\tStall occurs until cycle: " << cycle_no_dependency << endl;
						}
					}
				...
				if (!mem.hit_flag){
					if (print_flag) outputFile << endl << endl << "---------------------------------------------------------Stalled by Cache miss---------------------------------------------------------" << endl << endl;
					mem.hit_flag = true;
				}
				if (load_flag && ((forwarding_flag_int & forwarding_bit_MEM_int) || (forwarding_flag_float & forwarding_bit_MEM_float))){
					if (print_flag) outputFile << endl << endl << "---------------------------------------------------------Stalled by Memory---------------------------------------------------------" << endl << endl;
				}

				if (Flush_flag){
					if (print_flag) outputFile << endl << endl << "---------------------------------------------------------Flushed---------------------------------------------------------" << endl << endl;
					Flush_flag = false;
					flush_count++;
					cycle_no_dependency++;
				}
