...
unordered_map<string, vector<int>> Label_assign[4];
...
long long assemble(const std::string &line, unordered_map<string, int> &RFnames)
{
	...
	if (tokens[0] == ".local" || tokens[0] == ".data" || tokens[0] == ".size" || tokens[0] == ".global" || tokens[0] == ".section" || tokens[0] == ".type" || tokens[0] == ".bss" || tokens[0] == ".align" || tokens[0] == ".file" || tokens[0] == ".text" || tokens[0] == ".ident" || tokens[0] == ".section" || tokens[0] == ".p2align" || tokens[0] == ".asciz" || tokens[0] == ".byte" || tokens[0] == ".globl"){
		return 0;
	}
	...
	if (tokens[0] == ".word"){
		...
		Labels_pointer[Labels_vec.back()] = (0x10000/4) + data_section.size();
	...
	if (tokens[0] == ".comm"){
		Labels_pointer[tokens[1]] = (0x10000/4) + data_section.size();
	...
	if (tokens[0] == "add"){
		...
		if (tokens.size() == 2){
			Instruction *p = new Jalr<int>(1, RFnames[tokens[1]], 0);
			instructions.push_back(p);
			return (*p).assembler();
		}else{
			try{
		...
		Instruction *p = new Div<int>(RFnames[tokens[1]], RFnames[tokens[2]], RFnames[tokens[3]]);
		instructions.push_back(p);
		return (*p).assembler();
		...
		if (imm > 2047 || imm < -2048){
			Instruction *p = new Lui<int>(RFnames[tokens[1]], imm >> 12);
			Instruction *q = new Addi<int>(RFnames[tokens[1]], RFnames[tokens[1]], imm & 0xfff);
			long long q2 = (*q).assembler();
			instructions.push_back(p);
			instructions.push_back(q);
			return (*p).assembler() | (q2 << 32ll);
		}else{
			Instruction *p = new Addi<int>(RFnames[tokens[1]], 0, imm);
			instructions.push_back(p);
			return (*p).assembler();
		}
	}else if (tokens[0] == "mv"){
		Instruction *p = new Addi<int>(RFnames[tokens[1]], RFnames[tokens[2]], 0);
		instructions.push_back(p);
		return (*p).assembler();
	}else if (tokens[0] == "not"){
		Instruction *p = new Xori<int>(RFnames[tokens[1]], RFnames[tokens[2]], -1);
		instructions.push_back(p);
		return (*p).assembler();
	}else if (tokens[0] == "neg"){
		Instruction *p = new Sub<int>(RFnames[tokens[1]], 0, RFnames[tokens[2]]);
		instructions.push_back(p);
		return (*p).assembler();
	}else if (tokens[0] == "seqz"){
		Instruction *p = new Slti<unsigned int>(RFnames[tokens[1]], RFnames[tokens[2]], 1);
		instructions.push_back(p);
		return (*p).assembler();
	}else if (tokens[0] == "snez"){
		Instruction *p = new Slt<unsigned int>(RFnames[tokens[1]], 0, RFnames[tokens[2]]);
		instructions.push_back(p);
		return (*p).assembler();
	}else if (tokens[0] == "sltz"){
		Instruction *p = new Slt<int>(RFnames[tokens[1]], RFnames[tokens[2]], 0);
		instructions.push_back(p);
		return (*p).assembler();
	}else if (tokens[0] == "sgtz"){
		Instruction *p = new Slt<int>(RFnames[tokens[1]], 0, RFnames[tokens[2]]);
		instructions.push_back(p);
		return (*p).assembler();
		...
		Instruction *p = new Beq<int>(RFnames[tokens[1]], 0, stoi(tokens[2]));
		instructions.push_back(p);
		return (*p).assembler();
		...
		Instruction *p = new Bne<int>(RFnames[tokens[1]], 0, stoi(tokens[2]));
		instructions.push_back(p);
		return (*p).assembler();
		...
		Instruction *p = new Bge<int>(0, RFnames[tokens[1]], stoi(tokens[2]));
		instructions.push_back(p);
		return (*p).assembler();
		...
		Instruction *p = new Bge<int>(RFnames[tokens[1]], 0, stoi(tokens[2]));
		instructions.push_back(p);
		return (*p).assembler();
		...
		Instruction *p = new Blt<int>(RFnames[tokens[1]], 0, stoi(tokens[2]));
		instructions.push_back(p);
		return (*p).assembler();
		...
		Instruction *p = new Blt<int>(0, RFnames[tokens[1]], stoi(tokens[2]));
		instructions.push_back(p);
		return (*p).assembler();
		...
		Instruction *p = new Bge<int>(RFnames[tokens[2]], RFnames[tokens[1]], stoi(tokens[3]));
		instructions.push_back(p);
		return (*p).assembler();
		...
		Instruction *p = new Blt<int>(RFnames[tokens[2]], RFnames[tokens[1]], stoi(tokens[3]));
		instructions.push_back(p);
		return (*p).assembler();
		...
		Instruction *p = new Bge<unsigned int>(RFnames[tokens[2]], RFnames[tokens[1]], stoi(tokens[3]));
		instructions.push_back(p);
		return (*p).assembler();
		...
		Instruction *p = new Blt<unsigned int>(RFnames[tokens[2]], RFnames[tokens[1]], stoi(tokens[3]));
		instructions.push_back(p);
		return (*p).assembler();
	}else if (tokens[0] == "j"){
		try{
		...
		Instruction *p = new Jalr<int>(0, RFnames[tokens[1]], 0);
		instructions.push_back(p);
		return (*p).assembler();
		...
		Instruction *p = new Fsgnjn<float>(RFnames[tokens[1]], RFnames[tokens[2]], RFnames[tokens[2]]);
		instructions.push_back(p);
		return (*p).assembler();
	}else if (tokens[0] == "fmv.s"){
		Instruction *p = new Fsgnj<float>(RFnames[tokens[1]], RFnames[tokens[2]], RFnames[tokens[2]]);
		instructions.push_back(p);
		return (*p).assembler();
	}
	else{
...
int main(){
	...
	// 		(*instructions[Label_assign[1][Labels_vec[i]][j]]).operand1 = Labels_pointer[Labels_vec[i]];
	// 	}
	// }
	return 0;
}
