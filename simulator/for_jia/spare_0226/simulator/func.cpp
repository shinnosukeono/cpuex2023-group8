		...
		if (!fast_mode){
			...
			while (pc < 4 * (int)intrs.size() && pc != -1 && cycle < maxcount)
			{
				...
				if (EX != nullptr){
					...
					if (!dependency_penalty && load_flag && ((forwarding_flag_int & forwarding_bit_MEM_int) || (forwarding_flag_float & forwarding_bit_MEM_float))){
						cycle++;
						stall_count++;
					}
					...
					if (max_stall > cycle){
						stall_count++;
						cycle = max_stall;
						if (!dependency_penalty && print_mode > 0 && cycle >= print_cycle && cycle <= print_cycle_end){
				...
				if (!mem.hit_flag){
					if (print_mode > 0 && cycle-1 >= print_cycle && cycle-1 <= print_cycle_end) outputFile << endl << endl << "---------------------------------------------------------Stalled by Cache miss---------------------------------------------------------" << endl << endl;
					mem.hit_flag = true;
				}
				if (load_flag && ((forwarding_flag_int & forwarding_bit_MEM_int) || (forwarding_flag_float & forwarding_bit_MEM_float))){
					if (print_mode > 0 && cycle-1 >= print_cycle && cycle-1 <= print_cycle_end) outputFile << endl << endl << "---------------------------------------------------------Stalled by Memory---------------------------------------------------------" << endl << endl;
				}
